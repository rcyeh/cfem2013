...
if(dim(collect)[1]>0){
  ...
  one<-subset(collect,timegrp=="Early"&exchange!=''&exchange!='D',select=names)
  two<-subset(collect,timegrp=="Midday"&exchange!=''&exchange!='D',select=names)
  three<-subset(collect,timegrp=="Late"&exchange!=''&exchange!='D',select=names)
  write.csv(one, paste(date,"_early_spread_nonD.csv",sep=""))
  write.csv(two,paste(date,"_midday_spread_nonD.csv",sep=""))
  write.csv(two,paste(date,"_late_spread_nonD.csv",sep=""))
}

...
testSymbol =sample(Total,100,FALSE)
...
for (k in d){
  ...
  atbidask <- 0
  outsideBidAsk <- 0
l = c(1:length(testSymbol))    #Change this line to change to your listing
  for(ticker in l) {
  symbol = testSymbol[ticker]   #And change this line to change to your listing
  print(ticker)
  print(symbol)
  print(k)
  ...
  atmid <-atmid+ sum(a$effectiveHalfSpread[a$exchange=='D'] < 0.0001)
  midaskOrbidmid <- midaskOrbidmid+ sum(a$effectiveHalfSpread[a$exchange=='D'] >= 0.0001 & a$effectiveHalfSpread[a$exchange=='D'] < 0.9999)
  atbidask <-atbidask+ sum(abs(a$effectiveHalfSpread[a$exchange=='D'] - 1) < 0.0001)
  outsideBidAsk <-outsideBidAsk+ sum(a$effectiveHalfSpread[a$exchange=='D'] >= 1.0001)
  }
}
...
plot(outrange*100,xaxt = "n",xlab='Optimized delay in ms', ylab='percentage of trades outside bid/ask range')

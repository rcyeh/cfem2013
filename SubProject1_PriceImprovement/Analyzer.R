plot_forward_vs_regress_bin = function(bin = 0.2, 
  ...
  excludeD <- excludeDs[ddd]
  for (j in 1:length(times)){
    time <- times[j]
    file_name = paste('ExhaustiveAnalysis',date,'time',time,'_',useSOI,excludeD,'.csv',sep='')
    ...
    print(j)
    if(j ==1){
      plot(bingroups$Group.1*bin,bingroups$x,type='l',lwd = 2,col='yellow3',xlab ='SOI',ylab='return',main=paste('Return versus SOI pn 20130423',dep=''))
    }else{
